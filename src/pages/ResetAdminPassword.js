...
const ResetAdminPassword = () => {
  ...
  const handleReset = async (e) => {
    ...
    try {
      // await resetAdminPassword(formData.newPassword.trim(), token);
      const encryptedPassword = encryptFun(formData.newPassword.trim(), token); // assuming token contains username or email
      await resetAdminPassword(encryptedPassword, token);
      setSnackbar({
        open: true,
        message: "Password reset successful!",
        severity: "success",
