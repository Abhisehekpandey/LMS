      ...
      datasets: [
        {
          ...
          data: companyData.dataUsage.map(item =>
            Math.round(item.consumed / 1024)  // Remove decimals
          ),
        ...
        },
        {
          ...
          data: companyData.dataUsage.map(item =>
            Math.round((item.actual - item.consumed) / 1024)  // Remove decimals
          ),
          backgroundColor: companyData.dataUsage.map((item, index) =>
        ...
        },
        {
          label: 'Sold Storage',
          data: companyData.dataUsage.map(item =>
            Math.round((item.sold - item.actual) / 1024)  // Remove decimals
          ),
...
licenseAllocation: {
  labels: companyData.licenseAllocation.map((item, index) => `C${index + 1}`),
  datasets: [
    {
      label: 'Consumed (%)',
  ...
  costServer: {
    ...
    datasets: [
      ...
      },
      {
        label: 'Allocated',
        data: [
          Math.round((companyData.costServer.server1.allocated - companyData.costServer.server1.consumed) / 1024),
          Math.round((companyData.costServer.server2.allocated - companyData.costServer.server2.consumed) / 1024)
        ],
      ...
      },
      {
        ...
        data: [
          Math.round((companyData.costServer.server1.capacity - companyData.costServer.server1.allocated) / 1024),
          Math.round((companyData.costServer.server2.capacity - companyData.costServer.server2.allocated) / 1024)
        ],
        ...
        callbacks: {
          ...
          label: function (context) {
            ...
            if (datasetLabel === 'Actually Allocated') {
              ...
              const percentage = Math.round((dataPoint.actual / dataPoint.sold) * 100);
              return `Allocated: ${allocatedTB} TB `;
            }
  
            if (datasetLabel === 'Consumed Storage') {
              const consumedTB = Math.round(dataPoint.consumed / 1024);
              const percentage = Math.round((dataPoint.consumed / dataPoint.actual) * 100);
        ...
        callbacks: {
          ...
          label: function (context) {
            ...
            if (datasetLabel === 'Allocated (Total)') {
              const percentage = ((dataPoint.consumed / dataPoint.allocated) * 100).toFixed(1);
              const consumedPercentage = ((dataPoint.consumed / dataPoint.allocated) * 100).toFixed(1);
  const remainingPercentage = (100 - consumedPercentage).toFixed(1);
  const remainingLicenses = dataPoint.allocated - dataPoint.consumed;
  ...
  plugins: {
    ...
    datalabels: {
      ...
      formatter: function(value, context) {
        const shortName = companyData.licenseExpiration[context.dataIndex].shortName;
      ...
      callbacks: {
        ...
        label: function(context) {
          ...
          if (datasetLabel === 'Consumed Storage') {
            ...
            const companyInfo = companyData.dataUsage.find(c => c.shortName === company);
            
            if (companyInfo) {
              // Calculate allocated TB and percentage
              const allocatedTB = Math.round(companyInfo.actual / 1024);
              const consumedPercentage = Math.round((companyInfo.consumed / companyInfo.actual) * 100);
          ...
          if (datasetLabel === 'Available Capacity') {
            const availablePercentage = Math.round(((serverData.capacity - serverData.allocated) / serverData.capacity) * 100);
            const availableTB = Math.round((serverData.capacity - serverData.allocated) / 1024);
            // Show both companies and the available capacity
            const company1 = companies[0];
            const company2 = companies[1];
            const company1Info = companyData.dataUsage.find(c => c.shortName === company1);
            const company2Info = companyData.dataUsage.find(c => c.shortName === company2);
            
            // Create multiline tooltip with all information
            return [
              `Available: ${availableTB}TB`,
  ...
  return (
    <Box sx={{ display: "flex", position: "relative", height: "100vh", overflow: "hidden" }}>
      ...
      <Box sx={{ 
        ...
        overflow: "hidden",
        bgcolor: (theme) => theme.palette.mode === 'dark' ? '#121212' : '#f5f7fa'
      }}>
