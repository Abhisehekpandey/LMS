...
export default function UserTable() {
  ...
  const handleSaveChanges = async () => {
    try {
      ...
      if (editData.department) {
        ...
        if (editData.role) {
          ...
          if (editData.roleId) {
            ...
            roleObj =
              editRoleOptions.find(
                (r) =>
                  r.roleName?.toLowerCase() === editData.role?.toLowerCase(),
              ) ||
              deptObj.roles?.find(
                (r) =>
                  r.roleName?.toLowerCase() === editData.role?.toLowerCase(),
      ...
      if (deptObj && roleObj) {
        userPayload.deptId = deptObj.deptId || deptObj.id;
        userPayload.roleId = roleObj.roleId || roleObj.id;
      }
  ...
  useEffect(() => {
    if (editData?.department && editDialogOpen) {
      ...
      setEditRoleHasMore(true);
      setEditRoleSearch("");
      setEditRoleLoading(false); // Reset to allow fresh load
      loadEditRoles(1, "", true);
    } else {
      setEditRoleOptions([]);
    }
  }, [editData?.department, editDialogOpen]);

  // Debounce role search → reload from page 1 (ignoring initial empty search handled by above effect)
  useEffect(() => {
    if (!editDialogOpen || !editData?.department || editRoleSearch === "")
      return;
    const t = setTimeout(() => {
              ...
              <Grid item xs={12}>
                ...
                >
                  {editData.roles && editData.roles.length > 0 ? (
                    editData.roles.map((r, index) => (
                      <Chip
                        key={index}
                        label={`${r.roleName} (${r.department?.deptName || "N/A"})`}
                        size="small"
                        sx={{
                  ...
                  renderInput={(params) => (
                    <TextField
                      ...
                      label={
                        !editData.department
                          ? "Select Unit first"
                          : editRoleLoading && editRoleOptions.length === 0
                            ? "Loading roles..."
